...
export default function MazePage(): React.JSX.Element {
  ...
  const handleGameOver = useCallback(
    async (score: number) => {
      ...
      try {
        const res = await sansuApi.awardBadge(currentUser.id, newBadges, score, 'maze');
        ...
        {phase === 'over' ? (
          <section className="space-y-4 rounded-2xl bg-white p-6 text-center shadow-md dark:bg-gray-800">
            ...
            <button
              ...
              onClick={start}
              className="w-full rounded-xl bg-purple-600 py-4 text-lg font-bold text-white hover:bg-purple-700 disabled:opacity-60"
              data-testid="maze-again"
            >
              🪙 {SPEND_COSTS.play} で あたらしい めいろ
            </button>
            <Link
