...
## クリアごとに難しくなる（レベル制）
- [x] ブロックくずし: クリアで終わりにせず、レベル+1・ボール加速・ブロック復活で続行。落としたら終了。
- [x] めいろ: ゴールごとに大きい迷路へ。「やめる」で結果＝クリア迷路数。badge 1/5
- [x] 神経衰弱: 1ボードそろえるごとにカードが増える（5→8ペア）。「やめる」で結果＝クリアしたボード数。badge 1/4
- [x] (endless系)スネーク/ランナー/おちもの/ぱたぱたは速度漸増で既に難化、もぐらは制限時間あり
