...
> **状態（第1リリース）**: 第1弾コイン経済・第2弾ショップ・第3弾ミニゲーム基盤＋スネーク1本 完了。Z1/Z2 完了。
> 追加ゲーム（よけよけランナー他）は GAMES.md のカタログから後続ループで GR-A〜D… を積んで実装する。
...
- [x] Z2 → 完了(周17): PLAYBOOK の罠を LOOP_PROMPT の「検証チェックリスト」に昇格。BACKLOG に状態バナー、ドキュメント最終化
