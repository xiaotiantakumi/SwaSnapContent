...
## 周 20 — 専用きせかえ画面＋大きなアバターキャラ  (loop-version: v4)
- きっかけ: ユーザー「購入品はどこで装備？アバターみたいなのいないの？よくあるゲームを真似して」。
- やったこと: Web検索で着せ替えゲームUX（大キャラ＋カテゴリ＋タップ装備＋選択ハイライト＋drop-zone glow）を調査。
  `closet/page.tsx` 新規（AvatarDisplay xl で大アバター、ぼうし/はいけい/フレーム/うごき のカテゴリ別に所持アイテムを並べ、
  タップで equip/unequip、装備中は緑ハイライト、「🚫なし」で外す、おみせ導線）。AvatarDisplay に xl サイズ。
  ホームに🎨きせかえ（2x2グリッド化）、ショップ上部に「🎨きせかえで つけかえる」リンク＋購入メッセージ誘導。
- 検証: 全104緑、closet route200。iPhone(Playwright 390x844): 大アバター＋カテゴリ＋装備反映、console error 0。
  実機(computer-use): 🦄+👑+🌈背景+炎フレームの大アバター描画＋緑ハイライトを確認。
- 学び/罠: なし。Webリサーチ結果は出典付きでユーザーに提示。
- 残課題 / 次にやること: PR/staging へ push して実機(iPhone)確認可能に。
- prompt改善: なし。
- コミット: （この周で記録）
