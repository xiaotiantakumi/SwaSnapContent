import {
  ...
  PADDLE_W,
  type GS,
} from '../../app/sansu-100/games/logic/airhockey-logic';
...
function trackerMoveDir(gs: GS): number {
  const paddleCenter = gs.playerX + PADDLE_W / 2;
  const diff = gs.puckX - paddleCenter;
...
function runTrial(strategy: Strategy, seed: number): TrialResult {
  ...
  return {
    score: gs.playerScore,
    playerScore: gs.playerScore,
    cpuScore: gs.cpuScore,
    ticks,
    hits,
    goals,
...
function summarize(strategy: Strategy, results: TrialResult[]) {
  ...
  const playerScores = results.map((r) => r.playerScore);
  const cpuScores = results.map((r) => r.cpuScore);
  ...
  return {
    ...
    avgPlayerScore: Math.round(avg(playerScores) * 10) / 10,
    avgCpuScore: Math.round(avg(cpuScores) * 10) / 10,
    avgHits: Math.round(avg(hits) * 10) / 10,
    avgGoals: Math.round(avg(goals) * 10) / 10,
  };
}

function main(): void {
  console.log(`Air Hockey balance sim – ${TRIALS} trials × 2 strategies (tick limit ${TICK_LIMIT})\n`);
